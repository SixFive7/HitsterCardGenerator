...
**Current state:**
- Program.cs has Minimal API configured on port 5000
...
**Prior decisions:**
- Phase 5: Vite proxies `/api` to localhost:5000 during dev
...
- Check Network tab for failed API calls
  ...
  <verify>curl http://localhost:5000 returns HTML containing Svelte app; curl http://localhost:5000/api/health returns healthy status</verify>
  <done>App loads at localhost:5000, serves Svelte UI, and API endpoints respond correctly</done>
</task>
...
- [ ] http://localhost:5000 serves Svelte app (not 404)
