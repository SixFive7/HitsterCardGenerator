...
7. Configure to listen on port 5657 for Vite proxy compatibility
...
Note: Spectre.Console imports will cause warnings/errors as they're console-specific - that's expected and will be cleaned up when we fully migrate.
  ...
  <verify>dotnet build succeeds, dotnet run starts server on port 5657</verify>
  <done>Minimal API starts, listening on http://localhost:5657</done>
</task>
...
Register in Program.cs: app.MapHealthEndpoints()
  ...
  <verify>curl http://localhost:5657/api/health returns 200 with JSON response</verify>
...
- [ ] `curl http://localhost:5657/api/health` returns JSON with status "ok"
- [ ] OpenAPI docs available at /openapi/v1.json
</verification>

<success_criteria>
- Project uses Microsoft.NET.Sdk.Web
- Minimal API running on port 5657
