...
<task type="auto">
  ...
  <action>
Remove the hardcoded port configuration `app.Urls.Add("http://localhost:5657")` from Program.cs.
...
Delete only the line `app.Urls.Add("http://localhost:5657");` and its comment. Do NOT add any conditional logic - just remove it.
  </action>
